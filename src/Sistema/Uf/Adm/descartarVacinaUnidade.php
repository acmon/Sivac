...
$unidade->ExibirOpcaoOperacaoDeEstoque(8);

$unidade->ExibirFormularioDescartarVacinaUnidade();
	...
	if( isset($_POST['vacina'], $_POST['quantidade']) && empty($_POST['estornar'])) {
		...
		if ($unidade->VerificarEstoque($quantidade, $vacina_id, $unidade_id)) {
			...
			if ($idIncluido && $unidade->EstornarEstoque($unidade_id,
				...
				$unidade->ExibirMensagem("Serão descartadas
                                    $quantidade dose(s) da vacina $nomeVacina da unidade de
                                    saúde ". Html::FormatarMaiusculasMinusculas($nome_unidade) .
                    ...
                    $unidade->ExibirMensagem('Quantidade para descarte maior que o estoque atual!');
		}
	}
