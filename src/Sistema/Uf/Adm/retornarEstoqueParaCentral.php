...
$unidade->ExibirOpcaoOperacaoDeEstoque(3);
...
$unidade->ExibirFormularioRetornarEstoqueParaUnidadeCentral();
...
if(count($_POST)) {
	...
	$unidadeDeSaudeOrigem_id  = $_POST['unidadeCentral'];
	$unidadeDeSaudeDestino_id = $_POST['unidade'];
	$lote = $_POST['lote'];
	$obs = $_POST['obs'];
   ...
   // Verifica se os dados estão setados... 
	if( isset($_POST['vacina'], $_POST['quantidade']) && empty($_POST['estornar'])) {
		...
		if ($unidade->VerificarEstoque($quantidade, $vacina_id, $unidadeDeSaudeDestino_id)) {
		
			//... e se conseguiu inserir um transporte e salvar o estoque
			$idIncluido = $transporte->InserirTransporte($unidadeDeSaudeOrigem_id, $unidadeDeSaudeDestino_id, 
			                                   $vacina_id, $quantidade, $dataDeTransporte, $lote, 
			                                   $validadeDoLote, $obs, $codigoDeBarras, $produto);
			                                   
			if ($idIncluido && $unidade->SalvarEstoque($unidadeDeSaudeOrigem_id, $vacina_id, $quantidade)
			    
			    //usando o próprio estornar para retirar da unidade central
				&& $unidade->EstornarEstoque($unidadeDeSaudeDestino_id, $vacina_id, $quantidade)) {
				
				//buscar o nome da Unidade de Saúde
				$nome_unidade = $unidade->RetornarNomeUnidade($unidadeDeSaudeOrigem_id);	
                    ...
                    $unidade->ExibirMensagem("Serão retornadas $quantidade dose(s) da vacina $nomeVacina
					para a unidade de saúde ". Html::FormatarMaiusculasMinusculas($nome_unidade) .
					". Deseja mesmo efetuar essa operação? <br /> $botaoConfirmar  $botaoEstorno", 'Atenção',
                                        'onclick');
			}
		}
			...
			$unidade->ExibirMensagem('Quantidade para envio maior que o estoque atual!');
		}
	}
	...
	elseif( isset($_POST['estornar']) ) {
		...
		if ($unidade->VerificarEstoque($quantidade, $vacina_id, $unidadeDeSaudeOrigem_id)) {
                    ...
                    if($transporte->ExcluirTransporte($idIncluidoExtornar)
                       ...
                       && $unidade->SalvarEstoque($unidadeDeSaudeDestino_id, $vacina_id, $quantidade)

                       && $unidade->EstornarEstoque($unidadeDeSaudeOrigem_id, $vacina_id, $quantidade)) {
