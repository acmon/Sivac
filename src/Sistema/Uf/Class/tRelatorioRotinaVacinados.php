...
//------------------------------------------------------------------------------
     ...
     * Atributos
     */
     protected static $titulo = 'Indivíduos vacinados em rotina';

     protected static $nota = 'Este relatório tem por finalidade listar os indivíduos
                        vacinados em vacina de rotina,podendo o usuário do sistema
...
//------------------------------------------------------------------------------
	...
	public function ExibirRelatorio($pagina_atual = false)
    {
        ...
        $sql = 'SELECT COUNT( DISTINCT(usuario.id) ) '
             ...
             // Se o usuário escolheu ou não todas as doses:
             .     $sqlNumeroDaDose
        ...
        // Montando o cabeçalho para o número da dose:
        $arrCabecalho = Array();
    ...
    private function CalcularBarrasDePeriodo_outraAbordagem($dataInicial, $dataFinal)
    {
        ...
        if( $unidadeParaCalculo )
        {
            ...
            if( strpos($unidadeParaCalculo, 'ano(s)') !== false )
            {
                ...
                // Se o período da data final é menor do que o período final passado:
                if( $data->CompararData($periodos[$i-1]['dataFim'], '<', $dataFinal) )
                {
                 // Cria mais um período com menos de um ano:
                 $periodos[$i]['dataInicio'] = $data->IncrementarData($dataInicial, $i, 'year');
                ...
                {
                 // Reescreve a última data como período final:
                 $periodos[--$i]['dataFim'] = $dataFinal;
                }
            }
        ...
        // Período aceitos serão de 2 a 7, para a montagem do gráfico:
